...
function [x_vals, max_vals] = aboxplot(X,show_mean,widthl,widths,offset,fs,adjust_to_100, varargin)
offsets = [0.25 0.75];
...
for j=1:d
    ...
    for i=1:n
        ...
        if(adjust_to_100)
            line([i-widthl/2+(2*j-1-widthe)*widthl/(2*d)-offset i-widthl/2+(2*j-1+widthe)*widthl/(2*d)-offset],[min(fu,100) min(fu,100)],...
            'Color','k','HitTest','off','Parent',hgg(j));
           line([i-widthl/2+(2*j-1)*widthl/(2*d)-offset i-widthl/2+(2*j-1)*widthl/(2*d)-offset],[min(fu,100) fl],...
            'Color','k','LineStyle','-','HitTest','off','Parent',hgg(j));
        else
          line([i-widthl/2+(2*j-1)*widthl/(2*d)-offset i-widthl/2+(2*j-1)*widthl/(2*d)-offset],[fu fl],...
            'Color','k','LineStyle','-','HitTest','off','Parent',hgg(j));
          line([i-widthl/2+(2*j-1-widthe)*widthl/(2*d)-offset i-widthl/2+(2*j-1+widthe)*widthl/(2*d)-offset],[fu fu],...
            'Color','k','HitTest','off','Parent',hgg(j));  
        end
        line([i-widthl/2+(2*j-1-widthe)*widthl/(2*d)-offset i-widthl/2+(2*j-1+widthe)*widthl/(2*d)-offset],[fl fl],...
            'Color','k','HitTest','off','Parent',hgg(j));
        
        x_val = mean([i-widthl/2+(2*j-1-widthe)*widthl/(2*d)-offset i-widthl/2+(2*j-1+widthe)*widthl/(2*d)-offset]);
        x_vals(j,i)  = x_val;
        max_vals(j,i) =  fu;
        % Plot quantile
        if q3 > q1
            rectangle('Position',[i-widthl/2+(2*j-1-widths)*widthl/(2*d)-offset q1 widths*widthl/d q3-q1-offset],...
                'EdgeColor','w','FaceColor',colors{j,i},'HitTest','off','Parent',hgg(j));
        end
        ...
        if(show_mean)
            ...
            % Plot median
            line([i-widthl/2+(2*j-1-widths)*widthl/(2*d)-offset+0.001 i-widthl/2+(2*j-1+widths)*widthl/(2*d)-offset-0.001],[q2 q2],...
...
tick_vals = [1:n]';
tick_vals(1) = tick_vals(1) - offsets(1);
tick_vals(2:length(tick_vals)) = tick_vals(2:length(tick_vals)) - offsets(2);
...
xlim([min(tick_vals)-widths/2 max(tick_vals)+widths/2]);
