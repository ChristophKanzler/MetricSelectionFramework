...
function [x_vals, max_vals] = aboxplot_single(X,show_mean,varargin)
max_vals = [];
x_vals = [];
% Parameters
widthl = 0.7;
...
for j=1:d
    ...
    for i=1:n
        ...
        x_val = mean([i-widthl/2+(2*j-1-widthe)*widthl/(2*d) i-widthl/2+(2*j-1+widthe)*widthl/(2*d)]);
        x_vals(j,i)  = x_val;
        max_vals(j,i) =  fu;
        % Plot quantile
        if q3 > q1
...
set(gca,'XTick',0.675:0.05:1.325);
